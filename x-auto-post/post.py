...
# noteの有料記事シリーズへの誘導リンク
NOTE_URL = "https://note.com/ganmen_snow"

# 3投稿に1回、noteへの誘導CTAを含める
CTA_INTERVAL = 3
...
- {max_chars}文字以内（日本語・改行含む、URLは含めない）
- 絵文字・ハッシュタグは使わない
...
# 出力
投稿文のみを出力してください。前置き・後書き・説明文は一切不要です。URLは含めないでください。"""

# CTA（note誘導）を含める場合に追記する指示
CTA_INSTRUCTION = """- 最後の行に「もっと詳しく知りたい人はプロフィールのリンクから」のような、
  押し売り感のない一言を自然に加えてください（noteで詳しい記事を書いている、という文脈）"""
...
def generate_post(topic: str, recent_posts: list[str], with_cta: bool, max_chars: int) -> str:
    """ClaudeでX投稿文を生成"""
        ...
        messages=[
            {
                ...
                "content": PROMPT_TEMPLATE.format(
                    ...
                    max_chars=max_chars,
                    cta_instruction=CTA_INSTRUCTION if with_cta else "",
                ),
            }
        ],
    )
...
def main() -> None:
    ...
    # CTA_INTERVAL回に1回、noteへの誘導CTAを含める
    post_count = len(history) + 1
    with_cta = post_count % CTA_INTERVAL == 0
    # URL分（23文字 + 改行1文字）を考慮して上限を調整
    max_chars = 140 - 24 if with_cta else 140

    # 投稿文を生成
    recent_texts = [h["text"] for h in history]
    post_text = generate_post(topic, recent_texts, with_cta, max_chars)
    print(f"[生成された投稿]\n{post_text}\n")

    # 文字数超過チェック（Claudeがたまに超える場合のセーフティ）
    if len(post_text) > max_chars:
        print(f"⚠️ {max_chars}文字超過（{len(post_text)}文字）。再生成します。")
        post_text = generate_post(topic, recent_texts, with_cta, max_chars)
        if len(post_text) > max_chars:
            # それでも超えたら末尾を切る
            post_text = post_text[:max_chars - 1] + "…"
